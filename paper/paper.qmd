...
# Define color palette for different stages
stage_colors <- c("Early" = "#FFB6C1", "Intermediate" = "#FF69B4", "Advanced" = "#FF1493")

# Modify data to ensure correct ordering of stages
analysis_data$pathogenic_stage <- factor(analysis_data$pathogenic_stage, levels = c("Early", "Intermediate", "Advanced"))
...
# Plot the data
ggplot(pathogenic_stage_counts, aes(x = pathogenic_stage, y = percentage, fill = pathogenic_stage)) +
  geom_bar(stat = "identity") +
  labs(title = "Percentage of Patients by Pathogenic Stage",
       x = "Pathogenic Stage",
       y = "Percentage") +
  scale_fill_manual(values = stage_colors) +  # Apply color palette
  theme_minimal() +
  theme(
    text = element_text(family = "patrick")  # Set font to Patrick Hand
  )
