...
# Histogram of lung cancer patients survival time in days, post-diagnosis
...
# trying to represent percentage of pts by treatment type in a different way 

# Calculate counts and percentages of patients by treatment type
treatment_type_counts <- analysis_data %>%
  group_by(treatment_type) %>%
  summarise(count = n()) %>%
  mutate(percentage = count / sum(count) * 100)

# Pie chart for percentage of patients by treatment type
...
# Stacked bar chart for percentage of patients by treatment type
ggplot(treatment_type_counts, aes(x = "", y = percentage, fill = treatment_type)) +
...
# idea for the morning - sub in presence of distant metastasis, lymph node involvement and tumor size for pathogenic stage (frist graph)
