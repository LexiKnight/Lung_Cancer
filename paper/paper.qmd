...
#| message: false
#| label: fig-three
#| fig-cap: The histogram shows the percentage of lung cancer patients by treamtent type.
...
This illustrates that upon diagnosis, patients with more advanced pathologic stages are associated with a shorter survival time. Additionally, this suggests that patients receiving pharmaceutical therapy tend to exhibit a slower advancement in pathologic stage over time, thus increasing their survival time more so than those undergoing radiation therapy.
...
plot1 <- ggplot(data = analysis_data, aes(x = days_to_death, y = pathologic_stage, color = treatment_type)) +
  ...
  theme_minimal() +
  theme(text = element_text(family = "patrick", size = 14),  # Set font to Patrick Hand and increase the size
...
The trend line for radiation therapy is nearly a plateau, insinuating that radiation therapy was more effective in prolonging survival. This suggests that pharmaceutical therapy may  correlate with a lower likelihood of survival given distant metastasis over time compared to radiation therapy. Additionally, patients associated with higher classification of metastasis appear to have shorter survival times.
...
This suggests that pharmaceutical therapy and radiation therapy are both equally associated with increasing survival time based on lymph node involvement. Moreover, patients associated with higher classification of involvement with lymph nodes appear to have shorter survival times.
...
#| message: false
#| label: fig-six
#| fig-caption: The scatterplot demonstrates the relationship between lymph node involvement, time to death, and treatment type in lung cancer patients post-diagnosis.
...
This suggests that patients associated with larger tumors tend to have shorter survival times. This suggests that pharmaceutical therapy might be associated with a slower tumor growth rate and hence be more efficient in treating lung cancer patients in terms of tumor size. 
...
#| message: false
#| label: fig-seven
#| fig-caption: The scatterplot portrays the relationship between tumor size, time to death, and treatment type in lung cancer patients post-diagnosis.
